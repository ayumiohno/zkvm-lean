...
pub(crate) fn new_unique_hash_map<K, V>() -> UniqueHashMap<K, V> {
...
impl<'p> ExportFile<'p> {
    /// Original export-file Name index for a declaration at insertion index.
    pub fn declaration_name_index(&self, declar_index: usize) -> Option<u32> {
        self.declars
            .get_index(declar_index)
            .map(|(name, _)| u32::try_from(name.idx()).expect("name index does not fit u32"))
    }

    /// Look up a persistent declaration while recording the dependency for
    /// selective checking.  Most kernel lookups go through `Env`; the few
    /// structural checks which operate on `ExportFile` directly use this
    /// helper so they cannot escape the dependency trace.
    pub(crate) fn get_declar_traced(&self, n: &NamePtr<'p>) -> Option<&crate::env::Declar<'p>> {
        let (_idx, _, declar) = self.declars.get_full(n)?;
        #[cfg(feature = "touch_trace")]
        crate::touch_trace::note_touched(_idx);
        Some(declar)
    }

    /// As `get_declar_traced`, but return the declaration's insertion index.
    pub(crate) fn get_declar_index_traced(&self, n: &NamePtr<'p>) -> Option<usize> {
        let idx = self.declars.get_index_of(n)?;
        #[cfg(feature = "touch_trace")]
        crate::touch_trace::note_touched(idx);
        Some(idx)
    }
...
impl<'t, 'p: 't> TcCtx<'t, 'p> {
    ...
    pub fn read_name(&self, p: NamePtr<'t>) -> Name<'t> {
        match p.dag_marker() {
            DagMarker::ExportFile => {
                #[cfg(feature = "touch_trace")]
                crate::touch_trace::note_node(0, p.idx());
                self.export_file.dag.names.get_index(p.idx()).copied().unwrap()
            }
    ...
    pub fn read_level(&self, p: LevelPtr<'t>) -> Level<'t> {
        match p.dag_marker() {
            DagMarker::ExportFile => {
                #[cfg(feature = "touch_trace")]
                crate::touch_trace::note_node(1, p.idx());
                self.export_file.dag.levels.get_index(p.idx()).copied().unwrap()
            }
    ...
    pub fn read_expr(&self, p: ExprPtr<'t>) -> Expr<'t> {
        match p.dag_marker() {
            DagMarker::ExportFile => {
                #[cfg(feature = "touch_trace")]
                crate::touch_trace::note_node(2, p.idx());
                self.export_file.dag.exprs.get_index(p.idx()).copied().unwrap()
            }
...
impl Config {
    ...
    /// [lean-zkvm patch] Parse an export file from an arbitrary reader instead of a
    /// filesystem path or stdin. Needed because a zkVM guest has neither: the export
    /// file arrives as a byte frame from the host.
    pub fn to_export_file_from_reader<'a, R: std::io::BufRead>(
        self,
        reader: R,
