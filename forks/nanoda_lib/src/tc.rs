...
impl<'p> ExportFile<'p> {
    ...
    pub fn check_declar(&self, d: &Declar<'p>) {
        ...
        match d {
            ...
            Constructor(ctor_data) => {
                ...
                assert!(self.get_declar_traced(&ctor_data.inductive_name).is_some());
            }
            Recursor(recursor_data) => {
                ...
                match recursor_data.all_inductives.get(0) {
                    ...
                    Some(ind_name) => match self.get_declar_traced(ind_name) {
                        None => self.with_ctx(|ctx| {
                ...
                let recursor_idx = self.get_declar_index_traced(&recursor_data.info.name).unwrap();
                for ind_name in recursor_data.all_inductives.iter() {
                    match self.get_declar_index_traced(ind_name) {
                        None => self.with_ctx(|ctx| {
                            panic!(
    ...
    /// [lean-zkvm patch] Check only the declarations at index `skip` and later.
    ///
    /// Used for proof composition: the first `skip` declarations were already
    /// verified by a separate zkVM proof, so re-checking them is wasted work.
    /// The caller is responsible for binding `skip` to that proof (we do it by
    /// committing to a hash of the export file prefix those declarations came from).
    pub fn check_declars_skipping(&self, skip: usize) {
        for declar in self.declars.values().skip(skip) {
            self.check_declar(declar);
        }
    }

    /// Check the transitive dependency closure of `roots`, restricted to the
    /// first `declaration_limit` declarations.
    ///
    /// Dependencies are discovered by the checker itself, including implicit
    /// lookups performed during reduction and inductive validation.  The caller
    /// therefore does not have to trust an external dependency analyser.  The
    /// returned indices are sorted and identify exactly the declarations which
    /// were checked.
    #[cfg(feature = "touch_trace")]
    pub fn check_declar_closure(&self, roots: &[usize], declaration_limit: usize) -> Vec<usize> {
        assert!(declaration_limit <= self.declars.len(), "declaration limit is out of range");

        let mut queued = vec![false; declaration_limit];
        let mut checked = vec![false; declaration_limit];
        let mut work = VecDeque::new();
        for &idx in roots {
            assert!(idx < declaration_limit, "closure root is outside the certified prefix");
            if !queued[idx] {
                queued[idx] = true;
                work.push_back(idx);
            }
        ...
        while let Some(idx) = work.pop_front() {
            ...
            crate::touch_trace::reset();
            let (_, declar) = self.declars.get_index(idx).expect("closure index disappeared");
            self.check_declar(declar);
            checked[idx] = true;

            for dependency in crate::touch_trace::touched_indices() {
                assert!(
                    dependency < declaration_limit,
                    "certified declaration depends on a declaration outside the certified prefix"
                );
        ...
        checked.into_iter().enumerate().filter_map(|(idx, was_checked)| was_checked.then_some(idx)).collect()
    }
